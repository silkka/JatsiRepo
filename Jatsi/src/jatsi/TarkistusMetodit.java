...
public class TarkistusMetodit {
	 ...
	 * @param nopat			noppa taulukko
	 * @param yhdistelma	yhdistelmä jonka pisteet halutaan
	 * @return Halutun yhdistelmän pisteet; Palauttaa -1 jos yhdistelmää ei ole olemassa
	 */
	...
	public static int getKaksiPariaPisteet(int[] nopat){
	...
	public static int getKolmeSamaaPisteet(int[] nopat){
	...
	public static int getNeljaSamaaPisteet(int[] nopat){
		for(int i = 3; i<nopat.length; i++){
			if(nopat[i] == nopat[i-1] && nopat[i-2] == nopat[i-1] && nopat[i-2] == nopat[i-3]){
				return nopat[i]*4;
			}
		}
		return 0;
	...
	public static int getIsoSuoraPisteet(int[] nopat){
		if(nopat[0] != 2) return 0;
		for(int i = 1; i<nopat.length; i++){
	...
	public static int getPieniSuoraPisteet(int[] nopat){
		if(nopat[0] != 1) return 0;
		for(int i = 1; i<nopat.length; i++){
			if(nopat[i] != nopat[i-1] +1){
				return 0;
			}
	...
	public static int getTayskasiPisteet(int[] nopat){
		int kolmenSamanPisteet = getKolmeSamaaPisteet(nopat);
		if( kolmenSamanPisteet == 0) return 0;
		...
		if(parinPisteet != 0) return kolmenSamanPisteet + parinPisteet;
		else return 0;
	...
	public static int getSattumaPisteet(int[] nopat) {
		int summa = 0;
		for(int noppa : nopat){
			summa+=noppa;
		}
	...
	public static int getJatsiPisteet(int[] nopat){
		int jatsi = 50;
		for(int i = 1;i<nopat.length;i++){
			if(nopat[i] != nopat[i-1]) {
				jatsi = 0;
